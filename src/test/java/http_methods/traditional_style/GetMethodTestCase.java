...
public class GetMethodTestCase {
    ...
    @Test(priority = 1)
    public void getMethodWithUriApproach1(){
    ...
    @Test(priority = 2)
    public void getMethodWithUriApproach2(){
    ...
    @Test(priority = 3)
    public void getMethodWithUriApproach3(){

        String baseURI = "https://reqres.in";
        String basePath = "/api/users/2";
        RequestSpecification requestSpecification = RestAssured.given().baseUri(baseURI).basePath(basePath);
        Response response = requestSpecification.request(Method.GET);
        int statusCode = response.getStatusCode();
        int id = response.getBody().jsonPath().get("data.id");
        String responseBody = response.getBody().asString();
